  ...
  return (
		<nav className="h-16 w-full fixed top-0 bg-white z-10 text-neutral-800 border-b-2 shadow-sm flex justify-between items-center px-4">
			<div className="flex items-center gap-3">
				<h1 className="font-extrabold font-heading text-xl">ReSure</h1>
				<NavigationMenu>
					<NavigationMenuList>
						<NavigationMenuItem>
                ...
                {
                  FEATURES.filter(feature => feature.group === "Insurances").map((feature, index) => (
                    <>
                    <NavigationMenuLink href={feature.href} key={index}>
                      <div className="w-full h-full md:w-[400px] lg:w-[500px] px-4 py-2 space-y-2">
                        <h1 className="font-semibold flex items-center">
                          {feature.title} <ArrowRight className="h-4 w-4 ml-2"/>
                        </h1>
            ...
            <NavigationMenuItem>
              ...
              <NavigationMenuContent>
                {
                  FEATURES.filter(feature => feature.group === "Claims").map((feature, index) => (
                    <>
                    <NavigationMenuLink href={feature.href} key={index}>
                      <div className="w-full h-full md:w-[400px] lg:w-[500px] px-4 py-2 space-y-2">
                        <h1 className="font-semibold flex items-center">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger>
          <Avatar className="border-2 border-primary">
            <AvatarFallback className="bg-neutral-100 text-primary font-semibold">{currentUser.displayName?.charAt(0)}</AvatarFallback>
          </Avatar>
        </DropdownMenuTrigger>
        <DropdownMenuContent>
          <DropdownMenuItem asChild>
            <Button variant={"ghost"} onClick={() => signOut(auth)} className="w-full flex items-center gap-2">Log out <LogOut/></Button>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
		</nav>
